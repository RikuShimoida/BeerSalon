...
> このスキルは「PRのベースブランチ」と「リポジトリのデフォルトブランチ」を区別する。
> 両者が一致するプロジェクト（ベース = デフォルト）と、一致しないプロジェクト（例: 統合ブランチにPRを積み、
> デフォルトブランチは別）とで挙動が変わる点に注意する。以下では前者を「ベースブランチ」、後者を
> 「デフォルトブランチ」と呼ぶ。

### ブランチ作成

- gh コマンドを使用すること
- 修正前に必ずベースブランチから feature ブランチを作成する
  - 例: feature/profile-input

### コミット〜PR作成（順序厳守）

品質チェック（format / lint / 動作確認）が完了していることを前提に、以下を実行する。
...
- PR と Issue の紐付けは、PR 作成時に本文に `Closes #issue番号` を記載することで行う。
  - **PRのベースがデフォルトブランチでない場合、`Closes #N` を書いても GitHub による自動クローズは働かない**
    （自動クローズはデフォルトブランチ向けPRでのみ機能する）。この場合、マージ後に明示的なクローズが必要。
...
# ベースがデフォルトブランチでない場合は "Closes #123" による自動クローズが働かないので、Issue は明示的にクローズする。
