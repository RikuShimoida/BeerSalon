...
会話中で以下の用語を使用する場合、それぞれ対応するプロジェクトを指します：
...
**重要**: BeerSalonとBeerSalonAdminは**同じデータベースを共有**します。

- **BeerSalon**: フロントエンドアプリ（ユーザー向け）
- **BeerSalonAdmin**: 管理画面アプリ（バーオーナー・管理者向け）

両アプリケーションは同じSupabaseインスタンスに接続し、同じ`bars`テーブルやその他のマスターデータを共有します。

### ローカル開発環境でのSupabase設定

BeerSalonとBeerSalonAdminの両方が同じローカルSupabaseインスタンスを使用します：
...
- このポート番号をBeerSalonAdminでも共有することで、管理画面で追加したバーがBeerSalonでも表示されます

### なぜ同じデータベースを共有するのか

1. **データの一貫性**: BeerSalonAdminで登録したバー情報が即座にBeerSalonアプリに反映される
2. **マスターデータの共有**: ビールカテゴリ、国、地域、醸造所などのマスターデータを共有
3. **運用の簡素化**: 1つのデータベースを管理すればよい

### 注意事項

- BeerSalonプロジェクトでSupabaseを起動したら（`supabase start`）、そのポート番号を確認すること
- BeerSalonAdminの`.env.local`でも同じポート（54421）を指定すること
- 両アプリが異なるデータベースを参照すると、データが同期されません
- マイグレーションはBeerSalonプロジェクトで実行します
...
- ローカル開発サーバーは **必ずユーザーが起動する**
- Claude Code は **既存のポートを確認して利用する** のみ

---

# Claude Codeへの厳格なルール（違反厳禁）

## 禁止事項
- バックグラウンド実行 (`run_in_background: true`) の使用
