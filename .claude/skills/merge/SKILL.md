...
#### 1-1. CI ステータスチェック（全チェック成功が必須。満たさなければ停止）

```bash
gh pr checks $ARGUMENTS
```

- **全 CI チェックが成功（pass / SUCCESS）していることを必須とする**（2026-06-21 にユーザーと合意）。
  - `mergeable: MERGEABLE` は「コンフリクトが無い」ことしか保証せず、CI の合否は別物。
    コンフリクトが無くても CI が実行中・失敗中のままマージできてしまうため、ここで必ずガードする。
- 1つでも以下の状態があれば**マージせず停止し、状況を報告する**:
  - **実行中（pending / IN_PROGRESS / queued）** — まだ回り切っていない。
    「CI がまだ完了していないため待機が必要」と報告し、**少し置いてから再度 `/merge $ARGUMENTS` を実行するよう案内**する
    （このスキルは CI 完了を待ち続けない。延々と待って失敗時に止まれなくなる事故を避けるため）。
  - **失敗（fail / FAILURE / CANCELLED / TIMED_OUT 等）** — 赤いチェックがある。
    失敗したチェック名を挙げて報告し、**マージしない**。原因修正は別途（`/review-fix` 等）ユーザー判断に委ねる。
- `gh pr checks` は全チェック成功なら終了コード 0、未完了・失敗があれば非0で返るため、これも判定材料にしてよい。
- ステータスチェックが1つも設定されていない PR（CI 未設定リポジトリ等）の場合は、このガードをスキップしてよい
  （ただし本リポジトリは CI 必須運用のため、通常は該当しない）。
...
- **CI チェックが実行中・失敗のままマージすること**（全チェック成功が必須。1-1 参照）
