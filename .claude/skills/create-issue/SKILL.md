...
- **影響範囲**: 課題内容と設計ドキュメントから、既存機能・画面・データへの影響を特定して記載（不明な場合は推測せず「未確定事項」に回す）
...
### Phase 3.5: 着手ゲート充足チェック（feature 系のみ・提示前に必須）

**目的**: `/impl` 着手時の「要件確認の往復」を無くすため、着手に必要な情報を Issue 作成の時点で埋め切る。CLAUDE.md「実装着手前の要件確認義務」の4点＋受入条件が Issue 本文だけで満たされる状態にする。

Phase 4 でユーザーに本文を提示する **前に**、次のチェックリストを自己検査する。feature テンプレート（機能追加 / レイアウト変更 / アーキテクチャ変更）を使う場合に必須。bugfix はこのチェックの対象外（再現手順・影響範囲が既存項目でカバーされるため）。

- [ ] **対象画面**が特定できている（本体画面 / 管理画面 / 両方 のいずれか。「対象画面 / ルーティング」欄）
- [ ] **対象ページ・コンポーネント**が特定できている（ルーティングまで）
- [ ] **期待する動作**が「ユーザーが何をしたら何が起きるか」の粒度で書けている（「仕様（ユーザー視点）」「振る舞い・ロジック」欄）
- [ ] **影響範囲**が埋まっている（「影響範囲」欄。既存機能への影響・破壊的変更の有無まで）
- [ ] **受入条件**が Playwright で検証可能な形で1つ以上書けている

**埋まらない項目があった場合の扱い（推測補完は禁止）**:

- 入力ソース（課題No のセクション / Evernote ノート本文 / 設計ドキュメント）から読み取れない項目は、空欄のまま Issue 化しない。
- 不足項目を列挙して **その場でユーザーに質問し、回答を得てから本文に反映する**。この確認を Issue 作成時に前倒しすることで、`/impl` 着手時の往復が消える。
- どうしても確定できない項目（ユーザーも判断を保留したい等）は「未確定事項 / 確認事項」欄に明記し、`/impl` 側がそこを起点に確認できる状態にする。空欄で放置しない。
...
- 着手ゲート充足チェック（Phase 3.5）の必須項目を空欄のまま feature 系 Issue を作成すること（不足項目は提示前にユーザーへ確認する）
