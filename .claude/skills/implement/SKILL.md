...
完了報告の直後に `/understand` スキルの手順を実行する。`/understand` はまずモードを尋ねる（「いま解く」/「宿題にする」）。

- **「いま解く」を選んだ場合**: ユーザーが今回の実装内容を自分の言葉で説明できるか対話形式で確認する。理解チェックが完了するまで、このスキルは終了しない。
- **「宿題にする」を選んだ場合**: `/understand` が問いを宿題ファイル（`docs/understand-homework/`）へ書き出した時点で完了とみなし、このスキルを終了してよい（解き終えるまでブロックしない）。宿題は後から `/understand` を手動起動して取り組む。
