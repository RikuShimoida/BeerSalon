...
- `/understand` は今回の問いを宿題ファイル（`docs/understand-homework/`）へ書き出し、**さらに宿題用 Issue を自動起票する**。
- 書き出し（＋起票）が完了した時点でこのスキルは終了する（解き終えるまでブロックしない）。
  Issue 起票に失敗した場合も、Markdown 書き出しのみで完了とみなして終了してよい（implement を止めない）。
