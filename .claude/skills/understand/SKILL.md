...
- 厳密なフラグ機構は設けず、上記のスキル手順書ベースの運用で区別する
  （implement 側 Phase 2-4 にも「自動発動時は宿題用 Issue が起票される」前提を明記してある）。
...
- **「宿題にする」を選んだ場合** → 「Phase H: 宿題用 Issue の起票」を実行し、`/implement` をブロックせず終了する。
...
- `gh issue create`（または `gh label create`）が失敗した場合は、**起票せずにその旨を報告して終了**する。
  ...
  「Issue 起票に失敗。後で再度 `/understand` を実行して起票し直してください」と報告し、`/implement` はブロックしない。
- 自動発動（入口1）で起票に失敗しても implement を止めない。
